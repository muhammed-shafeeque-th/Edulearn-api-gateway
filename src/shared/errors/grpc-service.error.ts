...
export class GrpcServiceError extends BaseError {
  ...
  constructor(serviceError: ServiceError) {
    ...
    if (
      ...
      extractedReason = getMetadataValue(serviceError.metadata, 'reason');
    }

    // Fallbacks for errorCode and detail if not set in metadata
    const errorCode = // eslint-disable-next-line @typescript-eslint/no-unused-vars
      extractedErrorCode ||
